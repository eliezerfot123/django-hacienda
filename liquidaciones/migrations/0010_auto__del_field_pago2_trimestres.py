...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        call_command("loaddata", "cuotas.json")

        # Adding M2M table for field trimestres on 'Pago2'
        m2m_table_name = db.shorten_name(u'liquidaciones_pago2_trimestres')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('pago2', models.ForeignKey(orm[u'liquidaciones.pago2'], null=False)),
            ('cuota', models.ForeignKey(orm[u'liquidaciones.cuota'], null=False))
        ))
        db.create_unique(m2m_table_name, ['pago2_id', 'cuota_id'])
        db.execute("INSERT INTO liquidaciones_pago2_trimestres(pago2_id,cuota_id) SELECT pago2.id,cuota.id  from liquidaciones_pago2 as pago2,liquidaciones_cuota as cuota where orden <=trimestres and cuota.tipo='TRIM' AND pago2.id not in (SELECT liquidaciones_pago2_trimestres.pago2_id from liquidaciones_pago2_trimestres where pago2_id=pago2.id and cuota_id=cuota.id)")
        
        # Deleting field 'Pago2.trimestres'
        db.delete_column(u'liquidaciones_pago2', 'trimestres')


    def backwards(self, orm):
        # Adding field 'Pago2.trimestres'
        db.add_column(u'liquidaciones_pago2', 'trimestres',
                      self.gf('django.db.models.fields.IntegerField')(default=4),
                      keep_default=False)
        db.execute('UPDATE liquidaciones_pago2 SET trimestres=cuota.orden FROM (SELECT MAX(orden) AS orden,pago2_id FROM liquidaciones_pago2_trimestres GROUP BY pago2_id) AS cuota WHERE id=cuota.pago2_id')

        # Removing M2M table for field trimestres on 'Pago2'
        db.delete_table(db.shorten_name(u'liquidaciones_pago2_trimestres'))
    ...
    models = {
        ...
        u'liquidaciones.cuota': {
            'Meta': {'ordering': "['tipo', 'orden']", 'object_name': 'Cuota'},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        u'liquidaciones.pago2': {
            ...
            'trimestres': ('django.db.models.fields.related.ManyToManyField', [], {'to': u"orm['liquidaciones.Cuota']", 'symmetrical': 'False'}),
